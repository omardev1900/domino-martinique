...
import { Domino, GameState, PlayerId } from '../types';
import * as Engine from '../LogicEngine';
            ...
            setTimeout(() => {
              ...
              const dummyState: GameState = {
                ...
                players: distribution.players!.map((p, i) => ({
                  ...p,
                  wins: i === 0 ? 3 : (i === 1 ? 1 : 0) // p1=3, p2=1, p3=0
                }))
              };
      ...
      <div className="p-6 border-b border-stone-800 bg-stone-950 flex justify-between items-center flex-wrap gap-4">
        ...
        <button 
          ...
          className={`px-6 py-2 rounded-lg font-bold transition-all text-sm ${
            isSimulating ? 'bg-stone-800 text-stone-600 cursor-not-allowed' : 'bg-amber-600 hover:bg-amber-500 text-white shadow-[0_4px_0_rgb(146,64,14)] active:translate-y-1'
          }`}
        >
